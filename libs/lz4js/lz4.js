...
lz4.compressBlock = function compressBlock (src, dst, sIndex, sLength, hashTable) {
  ...
  if (sLength >= minLength) {
    ...
    while (sIndex < sEnd) {
      ...
      // in decoding.
      mLength = sIndex;
      while (sIndex < sEnd && src[sIndex] === src[mIndex]) {
        sIndex++;
        mIndex++;
      }
...
// Compresses data to an Lz4 block.
lz4.makeBlock = function makeBlock (src) {
  // Clear the hashtable.
  clearHashTable(hashTable);

  var maxSize = lz4.compressBound(src.length);
  var dst = lz4.makeBuffer(maxSize);
  var size = lz4.compressBlock(src, dst, 0, maxSize, hashTable);
