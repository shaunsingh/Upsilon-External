 ...
 * @return 0 if success, 1 if read failure
 * */
int load_rom(char* filename) {
    size_t file_len = 0;
    if (extapp_fileExists(filename, EXTAPP_FLASH_FILE_SYSTEM)) {
        const uint8_t* fp = (const uint8_t*) extapp_fileRead(filename, &file_len, EXTAPP_FLASH_FILE_SYSTEM);
        memcpy(&memory[0x200], fp, 0xFFF - 0x200);
    } else {
        return 1;
    }
    ...
    switch (op & 0xF000) {
        ...
        case 0x0000:
            switch (op & 0x00FF) {
                ...
                case 0x00EE:
                    debug_print("[OK] 0x00EE");
                    pc = stack[sp];
                    sp--;
                    pc += 2;
                    break;
                default:
                    debug_print("[FAILED] Unknown opcode! (0x00XX)");
                    break;
            }
            break;
        ...
        case 0x1000:
            debug_print("[OK] 0x1nnn");
            pc = op & 0x0FFF;
            break;

        // 2NNN: Calls subroutine at NNN
        case 0x2000:
            debug_print("[OK] 0x2nnn");
        ...
        case 0x8000:
            switch (op & 0x000F) {
                ...
                default:
                    debug_print("[FAILED] Unknown opcode! (0x8XXX)");
                    break;
            }
            break;
        ...
        case 0xE000:
            switch (op & 0x00FF) {
                ...
                // pressed
                case 0x009E:
                    debug_print("[OK] 0xEx9E");

                    if (kbd_handler() & keymappings[V[x]]) {
                        pc += 2;
                    }
                ...
                default:
                    debug_print("[FAILED] Unknown opcode! (0xEXXX)");
            }
            break;
        ...
        case 0xF000:
            switch (op & 0x00FF) {
                ...
                case 0x000A:
                    debug_print("[OK] 0xFx0A");
                    int key = extapp_getKey(true, NULL);

                    for (int i = 0; i < 16; i++) {
                        if (key = keymappings_await[i]) {
                            V[x] = i;
                            pc += 2;
                            break;
                        }
                ...
                default:
                    debug_print("[FAILED] Unknown op! (0xFXXX)");
                    break;
            }
            break;

        default:
            debug_print("[FAILED] Unknown op! (0xXXXX)");
            break;
    }
