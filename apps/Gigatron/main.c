...
void extapp_main() {
  ...
  for(uint16_t i = 0; i < 256; i++) {
    palette[i] = ((i << 14) & 0xc000) | ((i << 7) & 0x0600) | ((i >> 3) & 0x18);
  }

  uint16_t PC = 0;
  uint8_t IR = 0, D = 0, AC = 0, X = 0, Y = 0, OUT = 0, undef = 0, IN = 0xff, falling, prev_out = 0;
  uint8_t* RAM = malloc(1 << 15);
  int x = 0, y = 0;
  ...
  for (;;) {
    ...
    switch(IR) {
      ...
      case 0x75:
        Y = AC ^ RAM[D];
        break;const uint8_t VSYNC = 0x80;
const uint8_t HSYNC = 0x40;
...
#include <stdlib.h>
    ...
    if (falling & VSYNC) {
      ...
      if(k & SCANCODE_Left) IN |= BUTTON_LEFT;
      if(k & SCANCODE_Up) IN |= BUTTON_UP;
      if(k & SCANCODE_Down) IN |= BUTTON_DOWN;
      if(k & SCANCODE_Right) IN |= BUTTON_RIGHT;
      if(k & SCANCODE_OK) IN |= BUTTON_B;
      if(k & SCANCODE_Back) IN |= BUTTON_A;
      if(k & ((uint64_t) 1 << 8)) IN |= BUTTON_SELECT;
      if(k & SCANCODE_Home) IN |= BUTTON_START;
      if(k & SCANCODE_Home) IN |= BUTTON_START;
      if(k & SCANCODE_Zero) break;
      IN ^= 0xff;
    ...
    if ((y >= 34 && y < 480+34) && (x >= 12 && x < 160+12) && y%2 == 0) {
      extapp_pushRectUniform((x-12)*2, (y-34)/2, 2, 1, palette[OUT]);
    }
